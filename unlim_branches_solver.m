function [dens,success,params,sol] = unlim_branches_solver(params,options)
% obtains AV and lysosome densities for the unlimited fusion model
...
dens = calcTotalLysodistribution(params,dens,opt);

%% save lysosome data in convenient stacked form
branchpoints = [0 params.branchpoints params.L];

Ymaindata = [dens.Ya; dens.Yr; dens.Ysa; dens.Ysr];
...
for c = 1:nb+1
  ind = dens.mainind{c};
  xdata{c} = dens.xvals(ind)-branchpoints(c);
  
  Ydata{c}= Ymaindata(:,ind);
end
% collaterals
for c = 1:nb
  xdata{c+nb+1} = dens.xvals(dens.collind{c});
  
  Ydata{c+nb+1} = [dens.Yap{c}; dens.Yrp{c}; dens.Ysap{c}; dens.Ysrp{c}];
end


%% solve for unfused phagosomes and lysosomes using matlab's builtin PDE solver

% set up initial condition
xmesh = linspace(0,1,round(length(dens.xvals)/(nb+1)));
solinit = bvpinit(xmesh, @(x) guess_unlim_branches(x,params,dens));

if (any(isnan(solinit.y(:))))
  error('bad initial solution')
end
%% run solver
soloptions = bvpset('NMax',1000);
sol = bvp4c(@(x,y) diffeqs_unlim(x,y,params,Ydata,xdata), @(ya,yb) boundcond_unlim(ya,yb,params,dens), solinit,soloptions);
